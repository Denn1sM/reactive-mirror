...
const _Day = _interopRequireDefault(require('./Day'));
...
function Days(props) {
  ...
  const dayOfWeek = (0, _dates.getDayOfWeek)(activeStartDate, calendarType);
  ...
  const end = (function () {
    ...
    const daysInMonth = (0, _dateUtils.getDaysInMonth)(activeStartDate);

    if (showNeighboringMonth) {
      const activeEndDate = new Date();
      activeEndDate.setFullYear(year, monthIndex, daysInMonth);
      activeEndDate.setHours(0, 0, 0, 0);
      const daysUntilEndOfTheWeek = 7 - (0, _dates.getDayOfWeek)(activeEndDate, calendarType) - 1;
